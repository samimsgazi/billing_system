...
if(isset($_POST['submit'])){
	...
	$res = "INSERT INTO serv_bill_master (serv_bill_no,bill_date,CustomerName,Address1,City,Dist,CellNo,tax_id,taxPer,tax_amt,total_amt,Net_Amt,In_Word,referns) VALUES ('$billno','$billdate','$custName','$custAdd','$City','$Dist','$Ph','$tax','$taxper','$taxamt','$total','$Net','$InWord','$referns')";
	if(mysql_query($res))
	{
	//mysql_query("SELECT serv_bill_id FROM serv_bill_master WHERE serv_bill_no='$billname'");
	$id=mysql_fetch_assoc(mysql_query("SELECT id FROM serv_bill_master WHERE serv_bill_no='$billno'"));
	$varid=$id['id'];
...
foreach($qty as $a => $b)
//echo ++$i;
mysql_query( "INSERT INTO service_bill_trans (SlNo,serv_bill_id,serv_id,DoBy,serv_rate,serv_qty,serv_amt) VALUES ('$i','$billno','$service[$a]','$doby[$a]','$rate[$a]','$qty[$a]','$amt[$a]')");
//mysql_query($add);
...
$rec=mysql_fetch_assoc(mysql_query("SELECT * FROM serv_bill_master WHERE serv_bill_no='$billno'"));
$trans=mysql_query("SELECT * FROM service_bill_trans WHERE serv_bill_id='$rec[serv_bill_no]'");
...
?>
          ...
          Ph- <?php echo $com['ph_no']; ?><br>Email: <?php echo $com['email']; ?>
           ...
           <!-- End Company Details -->
           <table width="650px">
           <tr><td colspan="2" align="center" style="font-size:18px; "> Service Invoice</td></tr>
           <tr>
          ...
          <td align="right" style="font-size:small">E.&.O.E. </td>
          </tr>
          <tr>
          <td>Service Tax No: <?php echo $com['servcetax']; ?> </td>
          </tr>
            </tfoot>
            </table>
            </table>
   ...
   var x;
    if (confirm("Want to print bill..??") == true) {
       ...
       popupWin.document.write('<html><body onload="window.print()">' + divToPrint.innerHTML + '</html>');
		...
		window.location.href='service-bill-trans.php';
    } else {                  
        window.location.href='service-bill-trans.php';
    }
    </SCRIPT>

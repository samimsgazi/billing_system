...
$id=$_GET['id'];
$chk=mysql_query("SELECT * FROM serv_bill_master WHERE  id = '$id' ");
...
//if(!$_POST['name']="" && !$_POST['address'] ="" ){
  ...
  $res = "  UPDATE serv_bill_master SET serv_bill_no='$billno', bill_date='$billdate',CustomerName='$custName', Address1='$custAdd', City='$City',CellNo='$Ph', tax_id='$tax', tax_amt='$taxamt',taxPer='$taxper', total_amt='$total', Net_Amt='$Net', In_Word='$InWord' WHERE serv_bill_no='$billno'";
  
	if(mysql_query($res))
	{
		mysql_query("DELETE FROM service_bill_trans WHERE serv_bill_id='$billno'");
		$qty = $_POST['qty'];
		$rate = $_POST['rate'];
		$service = $_POST['Srvs'];
		 $amt = $_POST['amt'];
		...
		mysql_query( "INSERT INTO service_bill_trans (SlNo,serv_bill_id,serv_id,serv_rate,serv_qty,serv_amt) VALUES ('$i','$billno','$service[$a]','$rate[$a]','$qty[$a]','$amt[$a]')");
 ...
 <div align="center" style="overflow-y: scroll; height: 270px; " >
   ...
   <thead>
    ...
    <TABLE id="dataTable" width="550px" border="1">
        ...
        <?php
		$i=0;
		$trans=mysql_query("Select * from service_bill_trans where serv_bill_id='$rec[serv_bill_no]' order by SlNo");

 ...
 // return str;
  ...
  //alert(str);
    ...
    <ul>
        ...
        <li><a href="product-bill-trans.php" class="active">Product Bill</a></li>
...
<?php
 ...
 $cod['name'];
$chk=mysql_query("select * from prod_bill_master order by id");
...
?>
    <div class="main-content">
        ...
        <form class="form-basic" method="post" action="product-billdb.php">
        ...
        <h1>Product Bill </h1>
         ...
         <div class="form-row">
               ...
               <label>
                    <span>Bill No</span>
                    <input type="text" name="billno" id="billno" value="<?php echo $ncod?>" placeholder="Bill No" readonly required>
                </label>
            </div>
            <div class="form-row">
            <label>
...
?>
 ...
 <div align="center" style="overflow-y: scroll; height: 230px; " >
   ...
   <thead>
     <td align="center" width="195px">Product</td><td width="120px" align="center">By</td><td align="center" width="87px">Price</td>
    ...
    <TABLE id="dataTable" width="570px" border="1">
        ...
        <TR id='row_0'>
            ...
            <option>-------Select Product------</option>
                   <?php
$rs=mysql_query("Select DISTINCT prod_name from prod_master order by  prod_id");
	  while($row=mysql_fetch_array($rs))
{
  global $Srvs;
if($row[0]==$Srvs)
{
echo "<option value='$row[prod_name]' selected>$row[prod_name]</option>";
//echo '<option value="'.$row['name'].'"  data-price="'.$row['precio'].'">'.$row['name'].'</option>'';
}
else
{
...
?>
                    ...
                    <TD><select name="doby[]" id="doby" required>
           		 <option>--Company--</option>
                   <?php
$rs=mysql_query("SELECT DISTINCT madeby FROM prod_master ORDER By  prod_id");
	  while($row=mysql_fetch_array($rs))
{
if($row[0]==$Srvs)
{
echo "<option value='$row[madeby]' selected>$row[madeby]</option>";
//echo '<option value="'.$row['name'].'"  data-price="'.$row['precio'].'">'.$row['name'].'</option>'';
}
else
{
echo "<option value='$row[madeby]' >$row[madeby]</option>";
}
}
?>
                    </select></TD>
  ...
  <?php
			 	$Qry = "Select * from prod_bill_master order by id DESC";
				...
				while($row = mysql_fetch_array($sql))
				  ...
				  echo "<td align='Center'>" ; ?> <a href=edit/product-bill-edit.php?id=<?php echo $row['id']?> >Edit </a> <?php "</td>";
				  echo "<td align='Center'>" ; ?> <a href=edit/product-bill-delete.php?id=<?php echo $row['id']?>&bill=<?php echo $row['serv_bill_no']?> onclick="return confirm('Are you sure to delete?');" >Delete </a> <?php "</td>";
				  
				  echo "<td align='Center'>" ; ?> <a href=edit/product-bill-print.php?id=<?php echo $row['id']?>&bill=<?php echo $row['serv_bill_no']?> >Print </a> <?php "</td>";
